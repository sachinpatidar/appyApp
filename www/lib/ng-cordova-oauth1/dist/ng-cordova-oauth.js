    ...
    function oauthTwitter(clientId, clientSecret, options) {
      ...
      if(window.cordova) {
        if($cordovaOauthUtility.isInAppBrowserInstalled()) {
          ...
          alert(redirect_uri);
          alert(clientId + " clientId " + clientSecret + "  clientSecret  " + options + "   options")
            alert(typeof jsSHA)
          if(typeof jsSHA !== "undefined") {
            var oauthObject = {
              oauth_consumer_key: clientId,
            ...
            alert(JSON.stringify(oauthObject));
            ...
            $http({
              ...
              .success(function (requestTokenResult) {
                  ...
                  alert(JSON.stringify(requestTokenResult));
                var requestTokenParameters = (requestTokenResult).split("&");
                var parameterMap = {};
                for(var i = 0; i < requestTokenParameters.length; i++) {
                  parameterMap[requestTokenParameters[i].split("=")[0]] = requestTokenParameters[i].split("=")[1];
...
(function() {
  ...
  function cordovaOauthUtility($q) {
    ...
    function createSignature(method, endPoint, headerParameters, bodyParameters, secretKey, tokenSecret) {
        alert('method  ' + method + '    endpoint   ' + endPoint + ' headerParameters  ' + headerParameters + '  bodyParameters ' + bodyParameters + '   secretKey  ' + secretKey + ' tokenSecret ' + tokenSecret);
      if(typeof jsSHA !== "undefined") {
        var headerAndBodyParameters = angular.copy(headerParameters);
        var bodyParameterKeys = Object.keys(bodyParameters);
        alert(JSON.stringify(bodyParameterKeys));
        for(var i = 0; i < bodyParameterKeys.length; i++) {
          headerAndBodyParameters[bodyParameterKeys[i]] = encodeURIComponent(bodyParameters[bodyParameterKeys[i]]);
        }
        alert(JSON.stringify(bodyParameterKeys));
        var signatureBaseString = method + "&" + encodeURIComponent(endPoint) + "&";
        alert(JSON.stringify(signatureBaseString));
        var headerAndBodyParameterKeys = (Object.keys(headerAndBodyParameters)).sort();
        alert(JSON.stringify(headerAndBodyParameterKeys));
        ...
        alert(JSON.stringify({ signature_base_string: signatureBaseString, authorization_header: authorizationHeader, signature: headerParameters.oauth_signature }));
